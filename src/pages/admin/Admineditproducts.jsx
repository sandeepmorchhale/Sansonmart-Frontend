...
const Admineditproducts = () => {
    ...
    const updatedproducts = async (e) => {
    e.preventDefault(); // 👈 1. Page refresh rokne ke liye sabse zaroori

    if (!window.confirm("Are you sure you want to edit this product?")) return;

    try {
        // 👈 2. API call ka wait karein (await lagana zaroori hai)
        const res = await api.put(`/api/admin/products/update/${state._id}`, formdata);

        // 👈 3. Jab API success ho jaye, TAB alert aur navigate karein
        ...
        console.log("Any problem updating this product", error);
        alert("Update failed!"); // Error user ko bhi dikhana chahiye
    }
}
